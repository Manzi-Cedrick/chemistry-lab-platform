        ...
        <Section className='relative'>
            ...
            <div className='grid py-20 lg:grid-cols-2 sm:grid-cols-1 md:grid-cols-2 grid-cols-1 gap-6 place-items-center justify-center'>
                {ManagerArr && ManagerArr.map((service: IManage, index: number) => {
                    ...
                    const columnClasses = isLastElement ? "md:col-span-2" : "";

                    return (
                        <div
                            key={service.id}
                            className={`bg-white z-10 rounded-md md:max-w-[32vw] flex flex-row gap-6 place-items-center shadow-box w-[80vw] p-6 ${columnClasses}`}
                        >
